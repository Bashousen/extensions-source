...
class FilemoonExtractor(private val client: OkHttpClient) {
    ...
    private fun challengeFlow(
        ...
        val challengeData = try {
            ...
            response.body.string().let { json.decodeFromString<ChallengeResponse>(it) }
        ...
        ).let { json.encodeToString(it).toRequestBody(
            "application/json".toMediaType(),
        )}

        val attestData = try {
            val response = client.newCall(POST(attestUrl, apiHeaders, attestPayload)).execute()
            response.body.string().let { json.decodeFromString<AttestResponse>(it) }
        } catch (e: Exception) {
        ...
        val captchaData = try {
            ...
            response.body.string().let { json.decodeFromString<CaptchaResponse>(it) }
        } catch (e: Exception) {
        ...
        val verifyData = try {
            ...
            response.body.string().let { json.decodeFromString<VerifyResponse>(it) }
        } catch (e: Exception) {
        ...
        return try {
            ...
            response.body.string().let { json.decodeFromString<PlaybackResponse>(it) }
        } catch (e: Exception) {
...
inline fun <reified T> T.toJsonRequestBody(): RequestBody {
    return Json.encodeToString(serializer(), this).toRequestBody("application/json".toMediaType())
}
