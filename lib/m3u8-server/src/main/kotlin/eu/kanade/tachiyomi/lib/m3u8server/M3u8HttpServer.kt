...
class M3u8HttpServer(
    ...
    fun createLocalUrl(
        ...
        val sb = StringBuilder("http://127.0.0.1:$port/m3u8/video.m3u8?url=") //Dantotsu
    ...
    private fun buildChildM3u8Url(
        ...
        val sb = StringBuilder("http://127.0.0.1:$serverPort/m3u8?url=") //Dantotsu
            .append(URLEncoder.encode(m3u8Url, Charsets.UTF_8.name()))
        if (!referer.isNullOrBlank()) {
            sb.append("&referer=").append(URLEncoder.encode(referer, Charsets.UTF_8.name()))
        }
    ...
    private fun createLocalSegmentUrl(
        ...
        return buildString {
            append("http://127.0.0.1:$serverPort/segment?url=$encodedUrl") //Dantotsu
            if (key != null) {
                append("&key=")
