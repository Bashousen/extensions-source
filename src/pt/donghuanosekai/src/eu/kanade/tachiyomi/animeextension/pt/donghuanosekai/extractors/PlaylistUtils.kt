...
class PlaylistUtils(
    ...
    fun extractFromHls(
        ...
        val subtitleTracks = subtitleList + SUBTITLE_REGEX.findAll(masterPlaylist).mapNotNull {
            Track(
                fixUrl(it.groupValues[2], playlistUrl) ?: return@mapNotNull null,
                it.groupValues[1],
            )
        }.toList()
         ...
         *
         */
        return masterPlaylist.substringAfter(PLAYLIST_SEPARATOR).split(PLAYLIST_SEPARATOR)
            .mapNotNull { stream ->
                ...
                if (codec.isNullOrBlank()) {
                    ...
                    // if (codecs.all { it.startsWith("mp4a") })
                ...
                val videoUrl = stream.substringAfter("\n").substringBefore("\n").let { url ->
                    fixUrl(url, playlistUrl)?.trimEnd()
                } ?: return@mapNotNull null
    ...
    fun extractFromDash(
        ...
        // Get audio tracks
        val audioTracks =
            audioList + doc.select("Representation[mimetype~=audio]").map { audioSrc ->
    ...
    fun fixSubtitles(subtitleList: List<Track>): List<Track> =
        subtitleList.parallelMapNotNullBlocking {
            runCatching {
